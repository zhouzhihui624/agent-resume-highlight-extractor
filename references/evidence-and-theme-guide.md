...
“不按源码逐文件讲解”只代表正文不罗列文件和函数，不代表可以省略代码已经实现的对象、规则、状态和调用关系。根据主题类型至少还原下列内容：

| 机制类型 | 必须讲清的实现细节 |
| --- | --- |
| 状态与恢复 | 状态字段、状态转移、保存边界、恢复时如何重建上下文、重复副作用怎样避免 |
| 算法与检索 | 输入字段、召回与排序规则、阈值或预算、稳定排序、无结果和超预算时怎样兜底 |
| 协议与适配 | 请求正向转换、流式响应或工具调用的反向解析、错误和用量怎样归一、真实执行仍发生在哪一层 |
| 评测与回放 | Case 或 Spec 怎样冻结、Run 怎样隔离、Judge 怎样独立评审、结果怎样投票聚合并进入报告 |
| 工具与权限 | 工具怎样发现和暴露、调用怎样解析、权限与 Hook 在哪里生效、什么时候才允许真实副作用发生 |
| 版本与发布 | 草稿、候选、版本和发布态怎样转换、门禁依据什么、哪些制品不可变、失败后回到哪里 |

每个正式亮点至少给出一条正常执行链和一条失败或恢复链。链路必须能回答“上一步产出了什么、下一步拿什么继续、哪个组件做决定、状态在哪里改变、用什么证据证明”，否则只能标记为 `需补机制证据`，不能用通用 Agent 常识补齐。
